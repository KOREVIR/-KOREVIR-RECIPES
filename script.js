...
// ============================================================
// 50 TARİF – method'lar artık tek string olarak tutuluyor.
// openModal içinde splitMethod() ile adımlara ayrılacak.
// ============================================================
const RECIPES = [
    // 1
    {
...
// ============================================================
// YARDIMCI FONKSİYON: Method string'ini adımlara böler (8-9 adım)
// ============================================================
function splitMethod(methodStr) {
    // Nokta, ünlem, soru işareti ve ardından boşluk veya satır sonu ile böl
    let steps = methodStr.split(/[.!?]\s*/).filter(s => s.trim().length > 0);
    // Her adımı cümle haline getir (baş harfi büyük, sonuna nokta ekle)
    steps = steps.map(s => s.trim().charAt(0).toUpperCase() + s.trim().slice(1) + '.');
    // Eğer adım sayısı 8'den azsa, bazı adımları ikiye bölmeyi dene (virgül veya "and" ile)
    if (steps.length < 8) {
        let newSteps = [];
        for (let step of steps) {
            // Eğer adım çok uzunsa ve virgül içeriyorsa bölelim
            if (step.length > 60 && step.includes(',')) {
                let parts = step.split(',').map(p => p.trim());
                // Her parçayı cümle yap
    ...
    // Hala 8'den azsa, son adımı detaylandırarak tamamlayalım (zoraki)
    while (steps.length < 8) {
        let last = steps[steps.length - 1];
        if (last && last.length > 20) {
            // Son adımı noktadan böl
            let sub = last.split('. ').filter(s => s.trim().length > 0);
            if (sub.length > 1) {
                steps.pop();
                steps.push(...sub.map(s => s.trim().charAt(0).toUpperCase() + s.trim().slice(1) + '.'));
            } else {
                // Yapay bir adım ekle
                steps.push('Enjoy your perfectly cooked dish!');
                break;
            }
        } else {
            steps.push('Serve hot and enjoy with your loved ones!');
            break;
        }
    }
    // 9 adımdan fazla varsa, ilk 9'u al (isteğe bağlı)
    if (steps.length > 9) steps = steps.slice(0, 9);
    return steps;
}
...
// ============================================================
// OPEN MODAL (method adımları splitMethod ile bölünüyor, uzun ipuçları)
// ============================================================
function openModal(recipeId) {
    ...
    // Uzun ve bilgilendirici Chef's Tip'ler
    const chefTips = [
        'For an extra layer of flavour, we recommend marinating your protein overnight with your favourite herbs and spices. This allows the flavours to penetrate deeply, ensuring every bite is packed with taste. Don’t rush this step – patience truly pays off!',
        'Always bring your ingredients to room temperature before cooking, especially meat and dairy. This helps them cook more evenly and prevents the outside from burning while the inside remains underdone. A small habit that makes a huge difference.',
        'When frying, make sure your oil is hot enough before adding food. A simple test: drop a small piece of bread or batter into the oil – if it sizzles immediately, you’re good to go. This ensures a crispy, golden crust every time.',
        'Seasoning is not just about salt and pepper – think about layering flavours. Add a pinch of sugar to balance acidity, a squeeze of lemon to brighten, or a dash of smoked paprika for depth. Taste as you go and adjust.',
        'Don’t overcrowd the pan when frying or sautéing. If you add too much at once, the temperature drops and you’ll end up steaming rather than searing. Cook in batches if necessary for that perfect caramelisation.',
        'Resting meat after cooking is non-negotiable. It allows the juices to redistribute, making your steak, chicken, or lamb incredibly tender and moist. Cover loosely with foil and wait at least 5–10 minutes before carving.',
        'Fresh herbs are wonderful, but if you’re using dried ones, remember they are more concentrated – use about one-third of the amount you would for fresh. And always add dried herbs earlier in the cooking process to allow their flavours to bloom.',
        'Acidity is your secret weapon – a squeeze of lemon, a splash of vinegar, or a dash of wine can lift an entire dish, cutting through richness and adding a bright, refreshing note. Use it wisely and taste as you go.',
        'Caramelising onions slowly over low heat unlocks their natural sweetness and adds incredible depth to soups, stews, and sauces. Don’t rush this process – it can take 20–30 minutes, but the result is absolutely worth it.'
    ];
    const tip = chefTips[Math.floor(Math.random() * chefTips.length)];

    // Uzun ve keyifli Serving Suggestion'lar
    const servingSuggestions = [
        'This dish pairs beautifully with a crisp green salad dressed with a simple vinaigrette and a chilled glass of Sauvignon Blanc. The freshness of the salad complements the richness of the main, creating a perfectly balanced meal.',
        'For a truly memorable dining experience, serve alongside crusty artisan bread and a bowl of garlic butter. The bread is perfect for mopping up every last drop of sauce, and the garlic butter adds an extra layer of indulgence.',
        'We love serving this with creamy mashed potatoes or buttery couscous – both act as wonderful vehicles for the sauce. Add a side of steamed greens for colour and nutrition, and you’ve got a complete, wholesome feast.',
        'Garnish with a generous sprinkle of fresh parsley and a few lemon wedges on the side. The bright, citrusy aroma will awaken your senses and add a beautiful finish to the plate. It’s the little touches that make a meal unforgettable.',
        'For a lighter option, serve over a bed of quinoa or brown rice. The nuttiness of the grains pairs wonderfully with the dish, and you’ll feel satisfied without being weighed down. Perfect for a wholesome weeknight dinner.',
        'If you’re hosting a dinner party, present this dish on a large wooden board with scattered fresh herbs and edible flowers. It’s a stunning centrepiece that will impress your guests even before they take the first bite.',
        'Don’t forget a dollop of Greek yoghurt or sour cream on the side – its cool creaminess balances the heat and spices beautifully. Add a sprinkle of sumac or za’atar for an extra Middle Eastern twist.',
        'This recipe also works wonderfully as a filling for wraps or sandwiches the next day. Simply shred the leftover meat, add some crunchy slaw and a smear of sauce, and you have a delicious lunch ready in minutes.'
    ];
    ...
    // Method adımlarını böl
    const methodSteps = splitMethod(recipe.method);
    const methodList = methodSteps.map(step => `<li>${step}</li>`).join('');
